...
eyeShell compiles only JediTerm's `core` and `ui` source directories. The fork adds immutable retained-main-buffer snapshots, revisioned selection bounds, popup overrides, and coordinate-based search rendering. The standalone application, Pty4J, and local terminal support are not included.
